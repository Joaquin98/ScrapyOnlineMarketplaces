    ...
    def action_manager(self, response):
        ...
        for category in categories:
            try:
                ...
                action_parameters = action_parameters.replace(")", "").replace(
                    "'", "").replace(";", "").split(",")

                if action_name == "Dispara":
                    next_url = 'https://www.lagallega.com.ar/Menu-2.asp?Niv1=' + \
                        action_parameters[0]
                    yield scrapy.Request(url=next_url, callback=self.action_manager)
                elif action_name == "Dispara2":
                    next_url = 'https://www.lagallega.com.ar/Menu-3.asp?Niv1=' + \
                        action_parameters[0]+'&Niv2=' + action_parameters[1]
                    yield scrapy.Request(url=next_url, callback=self.action_manager)
                elif action_name == "Dispara3":
                    next_url = 'https://www.lagallega.com.ar/Menu-4.asp?Niv1=' + \
                        action_parameters[0]+'&Niv2=' + \
                        action_parameters[1] + '&Niv3=' + action_parameters[2]
                    yield scrapy.Request(url=next_url, callback=self.action_manager)
                    ...
                    next_url = 'https://www.lagallega.com.ar/Productos.asp?N1=' + \
    ...
    def parse_products(self, response):
        ...
        for product in products:
            try:
                ...
                product_item.add_xpath(
                    'name', 'div[@class = "InfoProd"]/div[@class = "InfoProdDet"]/div[@class = "desc"]/text()')
                product_item.add_xpath(
                    'price', 'div[@class = "InfoProd"]/div[@class = "precio"]/div[@class = "izq"]/text()')
                product_item.add_xpath(
                    'image', 'div[@class="MasInfo"]/@onclick')
        ...
        try:
            new_page = response.xpath(
                '//input[@type = "button" and @value = "Siguiente"]/@onclick').get()
            new_page = new_page.split(',')[1][2:]
            new_page = new_page.replace("'", "").replace(
                ")", "").replace(";", "")
